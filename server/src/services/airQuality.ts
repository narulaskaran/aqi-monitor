...
export async function getCoordinatesForZipCode(
  ...
  if (cachedCoordinates) {
    console.log(
      `Using expired cached coordinates for ZIP code: ${zipCode} while refreshing`,
    );
    ...
    fetchCoordinatesFromAPI(zipCode)
      ...
      .then(() =>
        console.log(`Background refresh completed for ZIP code: ${zipCode}`),
      )
      .catch((error) =>
        console.error(
          `Background refresh failed for ZIP code: ${zipCode}:`,
          error,
        ),
      );
 ...
 */
export async function fetchAndStoreAirQualityForZip(
  zipCode: string,
  ...
  try {
    ...
    await prisma.airQualityRecord.upsert({
      ...
      update: {
        ...
        pollutantData: airQualityData.pollutants
    ...
    console.log(
      `Successfully stored air quality data for ZIP code: ${zipCode}: AQI ${airQualityData.index}`,
    );
    ...
    if (sendNotifications) {
      try {
        ...
        console.log(
          `Sent ${alertsSent} air quality alert emails for ZIP code ${zipCode}`,
        );
      } catch (alertError) {
        console.error(
          `Error sending air quality alerts for ZIP code ${zipCode}:`,
          alertError,
        );
    ...
    console.error(
      `Error processing air quality for ZIP code ${zipCode}:`,
      error,
    );
    throw error;
  }
...
export async function getLatestAirQualityForZip(
  ...
  try {
    ...
    const pollutants = record.pollutantData
      ? (record.pollutantData as Record<
          string,
    ...
    console.error(
      `Error getting latest air quality for ZIP code ${zipCode}:`,
      error,
    );
    throw error;
  }
