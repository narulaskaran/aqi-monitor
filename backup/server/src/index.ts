...
// Import with dynamic import to support both development and production
const { addSubscription, getAllSubscriptions } = await (async () => {
  try {
    return await import("./db.js");
  } catch (e) {
    // Fall back for development
    if (process.env.NODE_ENV === "development") {
      // Remove the .js extension for ts-node in development
      const path = "./db";
...
// Import Twilio functions with dynamic import
const { sendVerificationCode, checkVerificationCode, initializeVerificationService } = await (async () => {
  try {
    return await import("./twilio.js");
  } catch (e) {
    // Fall back for development
    if (process.env.NODE_ENV === "development") {
      const path = "./twilio";
